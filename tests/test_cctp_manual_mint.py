...
def mock_wallet_service(mock_config):
    ...
    service._circle.get_transaction.side_effect = lambda tx_id: create_tx(
        tx_id, TransactionState.COMPLETE, "0xhash"
    )
    service._circle.create_contract_execution.return_value = create_tx(
        "tx-mint", TransactionState.PENDING
    )
...
async def test_execute_cctp_forces_mint_on_arc(mock_config, mock_wallet_service):
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        mock_response.json.return_value = {
            "messages": [{"status": "complete", "message": "0xmsg", "attestation": "0xsig"}]
        }
